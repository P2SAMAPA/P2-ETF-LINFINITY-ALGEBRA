...
from linfinity_algebra import linfinity_scores
...
def run_for_window(returns, window_days):
    ...
    try:
        raw_scores = linfinity_scores(
            ret_window,
            max_order=config.MAX_BRACKET_ORDER,
            eps=config.PERTURBATION_STRENGTH
        )
    ...
    top_etfs = [{"ticker": t, "linfinity_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "max_bracket_order": config.MAX_BRACKET_ORDER,
    ...
    out_file = f"output/linfinity_algebra_{timestamp}.json"
